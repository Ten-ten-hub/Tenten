...
public class AuthFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
        ...
        HttpServletRequest mutatedRequest = new HttpServletRequestWrapper(request){ // -> List로 변환해서 요소 추가 후 다시 Enumeration 반환
            @Override
            ...
            public Enumeration<String> getHeaders(String name){
                if("X-User-Id".equals(name)) return Collections.enumeration(List.of(finalUserId.toString()));
                if("X-User-Role".equals(name)) return Collections.enumeration(List.of(finalRole));
            ...
            public String getHeader(String name){ //헤더를 읽는 동작을 가로채서 JWT에서 꺼낸 값을 대신 반환
                if("X-User-Id".equals(name)) return finalUserId.toString();
                if("X-User-Role".equals(name)) return finalRole;
